...
var calculate = function() {
    if (n2 === null) {
        //如果现在n2为null,说明此时没有输入第二个数字，需要做的
        // 是交换变量，清空暂存器
        n2 = n1;
        reset();
    }else{
        //这种情况下可以肯定第一组数据完备，n1 n2都有值，
        // 如果还被按下运算符说明用户是要连续计算了
        switch(op){
            case '+':
                n2 += n1;
                reset();
...
EventHandlerUtil.addHandler(elementPro,'click',function(e) {
    ...
    if (target.tagName === "LI") {
        ...
        switch(targetValue){
            ...
            case 'sin':
                if (n1 !== 0) {
                    n2 = parseFloat(Math.sin(n1 * 2 * Math.PI/260).toFixed(6));
                    reset();
                    show(n2);
                }
                break;
            case 'cos':
                if (n1 !== 0) {
                    n2 = parseFloat(Math.cos(n1 * 2 * Math.PI/260).toFixed(6));
                    reset();
                    show(n2);
                }
                break;
            case 'tan':
                if (n1 !== 0) {
                    n2 = parseFloat(Math.tan(n1 * 2 * Math.PI/260).toFixed(6));
                    reset();
                    show(n2);
                }
                break;
